...
namespace UnityEditor.ShaderGraph
{
    ...
    class NewGraphAction : EndNameEditAction
    {
        ...
        public void AutoAction(string pathName)
        {
            var graph = new GraphData();
            graph.AddContexts();
            graph.InitializeOutputs(m_Targets, m_Blocks);
    ...
    static class GraphUtil
    {
        ...
        public static void CreateNewGraph(string name)
        {
            var graphItem = ScriptableObject.CreateInstance<NewGraphAction>();
            graphItem.targets = null;
            FileStream graphFile = File.Create(name+"."+ShaderGraphImporter.Extension);
            graphFile.Close();
            graphItem.AutoAction(name + "." + ShaderGraphImporter.Extension);
            //graphItem.Action(0, name, null);
            //ProjectWindowUtil.StartNameEditingIfProjectWindowExists(0, graphItem,string.Format("New Shader Graph.{0}", ShaderGraphImporter.Extension), null, null);
        }
        ...
        public static void CreateNewGraphWithOutputs(Target[] targets, BlockFieldDescriptor[] blockDescriptors)
        {
            ...
            {
                Debug.Log(t.displayName);
            }
            foreach(BlockFieldDescriptor b in blockDescriptors)
            {
                Debug.Log(b.name);
            }
            var graphItem = ScriptableObject.CreateInstance<NewGraphAction>();
        ...
        public static void CreateNewGraphWithOutputs(Target[] targets, BlockFieldDescriptor[] blockDescriptors, string path)
        {   
            /*
            foreach (Target t in targets)
            {
            ...
            */
            ...
            graphItem.targets = targets;
            graphItem.blocks = blockDescriptors;
            FileStream graphFile = File.Create(string.Format("{0}.{1}",path ,ShaderGraphImporter.Extension));
            graphFile.Close();
            graphItem.AutoAction(string.Format("{0}.{1}", path, ShaderGraphImporter.Extension));
        }
        ...
        // Opens the shadergraph file in text editor
