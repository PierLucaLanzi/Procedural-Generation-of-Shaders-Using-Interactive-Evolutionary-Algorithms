    ...
    class PreviewManager : IDisposable
    {
        ...
        void AddMasterPreview()
        {
            m_MasterRenderData = new PreviewRenderData
            {
                ...
                renderTexture =
                    new RenderTexture(400, 400, 32, RenderTextureFormat.ARGB32, RenderTextureReadWrite.Default)
                    {
        ...
        // This is only called for previews Inside the graph view. The Master preview is handled separately
        public void RenderPreviews(bool requestShaders = true)
        {
            ...
            using (var perMaterialPreviewProperties = PooledList<PreviewProperty>.Get())
            {
                ...
                int i = 0;
                // This is used to render 2d components in the graph view such as the procedural nodes and the maps
                ...
                if (renderMasterPreview)
                {
                    ...
                    // Here master preview render data is rendered
       ...
       private static readonly ProfilerMarker RenderPreviewMarker = new ProfilerMarker("RenderPreview");
        ...
        {
            //Debug.LogFormat("Rendering preview {0}",renderData.previewName);
            using (RenderPreviewMarker.Auto())
            {
                ...
                // Blit simply re-renders the texture more efficiently by keeping pixels that would be unchanged by the re-render
                ...
                if (renderData != m_MasterRenderData || !m_Graph.isOnlyVFXTarget)
                {
                    ...
                    Graphics.DrawMesh(mesh, transform, renderData.shaderData.mat, 1, m_SceneResources.camera, 0, m_SharedPreviewPropertyBlock, ShadowCastingMode.On, true, null, false);
                }
...
namespace AutoGen
{
    ...
    class ShaderPreviewManager : IDisposable
    {
        GraphData m_Graph;
        MessageManager m_Messenger;
        ...
        Dictionary<string, ShaderPrevRenderData> m_RenderDatas = new Dictionary<string, ShaderPrevRenderData>();  // stores all of the PreviewRendererData, mapped by node object ID
        ShaderPrevRenderData m_MasterRenderData;                                                               // cache ref to preview renderer data for the master node
        ...
        HashSet<ShaderPrevRenderData> m_PreviewsNeedsRecompile = new HashSet<ShaderPrevRenderData>();             // previews we need to recompile the preview shader
        HashSet<ShaderPrevRenderData> m_PreviewsCompiling = new HashSet<ShaderPrevRenderData>();                  // previews currently being compiled
        HashSet<ShaderPrevRenderData> m_PreviewsToDraw = new HashSet<ShaderPrevRenderData>();                     // previews to re-render the texture (either because shader compile changed or property changed)
        HashSet<ShaderPrevRenderData> m_TimedPreviews = new HashSet<ShaderPrevRenderData>();                      // previews that are dependent on a time node -- i.e. animated / need to redraw every frame

        bool m_TopologyDirty;                                                                               // indicates topology changed, used to rebuild timed node list and preview type (2D/3D) inheritance.

        HashSet<BlockNode> m_MasterNodeTempBlocks = new HashSet<BlockNode>();                               // temp blocks used by the most recent master node preview generation.
        
        AbstractPreviewSceneResources m_SceneResources;
        Texture2D m_ErrorTexture;
        Vector2? m_NewMasterPreviewSize;
        ...
        {
            get => _activeStageIndex;
            set
            {
                _activeStageIndex = StageSelector.getUsableStageIndex(value);
                m_SceneResources.closeScene();
                m_SceneResources = StageSelector.getStage(_activeStageIndex);
            }
        }

        public ShaderPrevRenderData masterRenderData
        {
            get { return m_MasterRenderData; }
        }

        public ShaderPreviewManager(GraphData graph, MessageManager messenger)
        {
            m_SharedPreviewPropertyBlock = new MaterialPropertyBlock();
            m_Graph = graph;
            m_Messenger = messenger;
            ...
            // This ensures that the input values to each node are taken into account
            // If disabled, default values are loaded
            foreach (var node in m_Graph.GetNodes<AbstractMaterialNode>())
                AddPreview(node);
        ...
        public ShaderPrevRenderData GetShaderPrevRenderData(AbstractMaterialNode node)
        {
            ShaderPrevRenderData result = null;
            if (node == kMasterProxyNode ||
                node is BlockNode ||
                node == m_Graph.outputNode) // the outputNode, if it exists, is mapped to master
            {
        ...
        void AddMasterPreview()
        {
            m_MasterRenderData = new ShaderPrevRenderData
            {
                previewName = "Master Preview",
                renderTexture =
                    new RenderTexture(400, 400, 32, RenderTextureFormat.ARGB32, RenderTextureReadWrite.Default)
        ...
        void AddPreview(AbstractMaterialNode node)
        {
            ...
            var renderData = new ShaderPrevRenderData
            {
                previewName = node.name ?? "UNNAMED NODE",
                renderTexture =
                    new RenderTexture(200, 200, 16, RenderTextureFormat.ARGB32, RenderTextureReadWrite.Default)
        ...
        void CollectPreviewProperties(IEnumerable<AbstractMaterialNode> nodesToCollect, PooledList<PreviewProperty> perMaterialPreviewProperties)
        {
            ...
            using (var tempPreviewProps = PooledList<PreviewProperty>.Get())
            {
                ...
                {
                    /*
                    Debug.LogFormat("Setting {0} in shader",previewProperty.name);
                    try
                    {
                        Debug.LogFormat("{0}", m_MasterRenderData.shaderData.mat.GetVector(previewProperty.name));
                    }
                    catch
                    {
        ...
        public void RenderPreviews(bool requestShaders = true)
        {
            if (m_SceneResources == null) return;
            //Debug.Log("Called RenderPreviews");
            using (RenderPreviewsMarker.Auto())
            using (var renderList2D = PooledList<ShaderPrevRenderData>.Get())
            using (var renderList3D = PooledList<ShaderPrevRenderData>.Get())
            using (var nodesToDraw = PooledHashSet<AbstractMaterialNode>.Get())
            using (var perMaterialPreviewProperties = PooledList<PreviewProperty>.Get())
            {
                // update topology cached data
                ...
                if (renderMasterPreview)
                {
                    //Debug.Log("Rendering Mater Preview in Alternate preview manager");
                    if (m_NewMasterPreviewSize.HasValue)
                    {
                        if (masterRenderData.renderTexture != null)
                            Object.DestroyImmediate(masterRenderData.renderTexture, true);
                    ...
                    // If no mesh has been serialized, set mesh to default (any non default mesh is serialized to keep that mesh as preview at next opening)
                    var mesh = m_Graph.previewData.serializedMesh.mesh ? m_Graph.previewData.serializedMesh.mesh : m_SceneResources.sphere;
                    var previewTransform = Matrix4x4.Rotate(m_Graph.previewData.rotation);
                    var scale = m_Graph.previewData.scale;
                    previewTransform *= Matrix4x4.Scale(scale * Vector3.one * (Vector3.one).magnitude / mesh.bounds.size.magnitude);
        ...
        void ProcessCompletedShaderCompilations()
        {
            // Check for shaders that finished compiling and set them to redraw
            using (ProcessCompletedShaderCompilationsMarker.Auto())
            using (var previewsCompiled = PooledHashSet<ShaderPrevRenderData>.Get())
            {
                foreach (var preview in m_PreviewsCompiling)
                {
                    {
                    ...
                    ShaderPrevRenderData renderData = preview;
                    PreviewShaderData shaderData = renderData.shaderData;
        ...
        void KickOffShaderCompilations()
        {
            // Start compilation for nodes that need to recompile
            using (KickOffShaderCompilationsMarker.Auto())
            using (var previewsToCompile = PooledHashSet<ShaderPrevRenderData>.Get())
            {
                // master node compile is first in the priority list, as it takes longer than the other previews
                if (m_PreviewsCompiling.Count + previewsToCompile.Count < m_MaxPreviewsCompiling)
                {
        ...
        private static readonly ProfilerMarker BeginCompileMarker = new ProfilerMarker("BeginCompile");
        void BeginCompile(ShaderPrevRenderData renderData, string shaderStr)
        {
            using (BeginCompileMarker.Auto())
            {
                var shaderData = renderData.shaderData;
        ...
        private void ForEachNodesPreview(
            IEnumerable<AbstractMaterialNode> nodes,
            Action<ShaderPrevRenderData> action)
        {
            foreach (var node in nodes)
            {
                var preview = GetShaderPrevRenderData(node);
                if (preview != null)    // some output nodes may have no preview
                    action(preview);
            }
        }
        ...
        void UpdateTopology()
        {
            ...
            ProcessUpstreamNodesInDependencyOrder(
                ...
                (node, children) =>
                {
                    ...
                    if (preview.previewMode == PreviewMode.Inherit)
                    {
                        if (children.Any(child => GetShaderPrevRenderData(child).previewMode == PreviewMode.Preview3D))
                            preview.previewMode = PreviewMode.Preview3D;
                        else
                            preview.previewMode = PreviewMode.Preview2D;
                    }
        ...
        private static readonly ProfilerMarker RenderPreviewMarker = new ProfilerMarker("RenderPreview");
        void RenderPreview(ShaderPrevRenderData renderData, Mesh mesh, Matrix4x4 transform, PooledList<PreviewProperty> perMaterialPreviewProperties)
        {
            //Debug.LogFormat("Rendering preview {0}", renderData.previewName);
            using (RenderPreviewMarker.Auto())
            {
                ...
                if (renderData != m_MasterRenderData || !m_Graph.isOnlyVFXTarget)
                {
                    m_SceneResources.camera.targetTexture = temp;
                    Graphics.DrawMesh(mesh, transform, renderData.shaderData.mat, 1, m_SceneResources.camera, 0, m_SharedPreviewPropertyBlock, ShadowCastingMode.On, true , null, false);
                    m_SceneResources.addMeshForShadows(mesh, transform);
                    m_SceneResources.updateScene();
                }
        ...
        void InitializeSRPIfNeeded()
        {
            ...
            // issue a dummy SRP render to force SRP initialization, use the master node texture
            ShaderPrevRenderData renderData = m_MasterRenderData;
            var previousRenderTexture = RenderTexture.active;
        ...
        void DestroyRenderData(ShaderPrevRenderData renderData)
        {
            if (renderData.shaderData != null)
            {
                if (renderData.shaderData.mat != null)
