...
namespace UnityEditor.ShaderGraph.Drawing
{
    ...
    class SearchWindowProvider : ScriptableObject
    {
        ...
        {
            //Debug.Log("Here the node adder is generated");
        ...
        public void GenerateNodeEntries()
        {
            ...
            foreach (var type in NodeClassCache.knownNodeTypes)
            {
                ...
                if (titleAttribute != null)
                {
                    ...
                    //Debug.Log(string.Format("Checking compatibility with node"));
                    if (ShaderGraphPreferences.allowDeprecatedBehaviors && node.latestVersion > 0)
                    {
                        ...
                        foreach (int i in versions)
                        {
                            ...
                            Debug.LogFormat("Adding {0}", depNode.name);
                            if (multiple)
                            {
            ...
            SortEntries(nodeEntries);
            currentNodeEntries = nodeEntries;
            //Debug.Log("Generated node entries");
            Profiler.EndSample();
        }
        ...
        // Here compatibility is checked directly with class variables, which must be updated somewhere
        void AddEntries(AbstractMaterialNode node, string[] title, List<NodeEntry> addNodeEntries)
        {
            ...
            // Checking compatibility here
            var connectedSlot = connectedPort.slot;
            
            //Debug.LogFormat("Connected slot is {0}", connectedSlot.displayName);
            /*if(node is BlockNode)
            {
                if (node.name.Contains("Normal"))
                {
                    Debug.LogFormat("Evalutating for {0}", node.name);
                }
            }*/

            m_Slots.Clear();
            // Gets all the slots from the checked node
            node.GetSlots(m_Slots);
            var hasSingleSlot = m_Slots.Count(s => s.isOutputSlot != connectedSlot.isOutputSlot) == 1;
            // removes all the slots from the checked node not compatible with the one in output
            ...
            // Add all the remaining to the entries 
    ...
    class SearcherProvider : SearchWindowProvider
    {
        ...
        public bool OnSearcherSelectEntry(SearcherItem entry, Vector2 screenMousePosition)
        {
            ...
            */
            ...
            //Debug.Log(string.Format("Chosen node {0}", node.name));
